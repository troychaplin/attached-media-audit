...
export default function useScanProgress( { onComplete } = {} ) {
	const [ state, setState ] = useState(
		() => window.wpAttachedMediaAudit?.initialProgress || { status: 'idle', progress: 0, total: 0 }
	);
	...
	const poll = useCallback( () => {
		...
		fetch( `${ ajaxUrl }?action=attached_media_audit_progress&nonce=${ nonce }` )
			.then( ( r ) => r.json() )
			.then( ( json ) => {
	...
	const startScan = useCallback( () => {
		...
		body.append( 'action', 'attached_media_audit_scan' );
